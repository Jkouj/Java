...
public class LoopDrills
{
   ...
   {  
      ...
      //    value stored in x
      x++;
      System.out.println("x++ adds one to the existing x value, so x is now "+x);
      
      //d) use System.out.println to determine the effect of this statement on the 
      //    value stored in x
      x--;
      System.out.println("x++ subtracts one from the existing x value, so x is now "+x);
    ...
    public static void C2FTable(int startC, int stopC, int stepC){
       ...
       public static void miles2kmTable(int startM, int stopM, double stepM){
        double miles = startM;
        double km;
        while (miles <= stopM) {
             km = miles*1.609344;
